...
struct ImVec2
{
    ...
    bool operator==(ImVec2 vec) { return (x == vec.x && y == vec.y); }
    ImVec2 operator+(ImVec2 vec) { return ImVec2(x + vec.x, y + vec.y); }
    ImVec2 operator*(float n) { return ImVec2(x * n, y * n); }
    ImVec2 operator*(ImVec2 vec) { return ImVec2(x * vec.x, y * vec.x); }
...
namespace ImGui
{
    ...
    IMGUI_API bool          Combo(const char* label, int* current_item, const char* const items[], int items_count, int popup_max_height_in_items = -1, ImGuiComboFlags flags = 0);
    IMGUI_API bool          Combo(const char* label, int* current_item, const char* items_separated_by_zeros, int popup_max_height_in_items = -1, ImGuiComboFlags flags = 0);      // Separate items with \0 within a string, end item-list with \0\0. e.g. "One\0Two\0Three\0"
    IMGUI_API bool          Combo(const char* label, int* current_item, bool(*items_getter)(void* data, int idx, const char** out_text), void* data, int items_count, int popup_max_height_in_items = -1, ImGuiComboFlags flags = 0);
...
enum ImGuiTreeNodeFlags_
{
    ...
    // MINE
    ImGuiTreeNodeFlags_NoOpenOnHoverDragDrop =  1 << 14,
